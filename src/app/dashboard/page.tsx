...
export default function DashboardPage() {
  ...
  return <main id="main-content" className="mx-auto grid flex-1 max-w-2xl content-center gap-6 px-6"><div><h1 className="text-2xl font-semibold">Dashboard</h1><p className="mt-2">Signed in as <strong>{user.email}</strong>.</p><p className="text-text-muted">Role: {role}</p></div><button type="button" onClick={handleSignOut} disabled={signingOut} className="rounded border-border bg-surface px-4 py-2 text-primary hover:bg-surface-subtle disabled:opacity-60">{signingOut ? "Signing out..." : "Logout"}</button></main>;
}
