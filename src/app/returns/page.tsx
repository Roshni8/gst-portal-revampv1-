...
function OutwardView({ data, busy, error, onImport, onUpload, onFinalise }: { data: OutwardData; busy: boolean; error?: string; onImport: () => void; onUpload: (event: React.FormEvent<HTMLFormElement>) => Promise<boolean>; onFinalise: () => void }) {
  ...
  return <section className="gst-ims-review is-reference-layout gst-gstr1-filing">
    ...
    <section className="gst-reconciliation-guide" aria-labelledby="reconciliation-guide-heading">
      <div><p className="gst-return-form-label">ERP and e-invoice reconciliation</p><h3 id="reconciliation-guide-heading">Match status guide</h3></div>
      <div className="gst-ims-table-wrap"><table className="gst-ims-table gst-reconciliation-guide-table"><thead><tr><th>Status</th><th>Match condition</th></tr></thead><tbody><tr><th><span className="gst-ims-status is-approved">Matched</span></th><td>Invoice type, normalised invoice number and date match; GSTIN, place of supply and all amounts agree within ₹0.01.</td></tr><tr><th><span className="gst-ims-status is-pending">Amount mismatch</span></th><td>The invoice matches, but total value, taxable value or any tax amount differs by more than ₹0.01.</td></tr><tr><th><span className="gst-ims-status is-pending">Field mismatch</span></th><td>The invoice and amounts match, but the recipient GSTIN or place of supply differs.</td></tr><tr><th><span className="gst-ims-status is-pending">ERP only</span></th><td>An ERP sales invoice has no matching imported e-invoice.</td></tr><tr><th><span className="gst-ims-status is-pending">E-invoice only</span></th><td>An imported e-invoice has no matching ERP sales invoice.</td></tr></tbody></table></div>
    </section>
